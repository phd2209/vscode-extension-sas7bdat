...
export default abstract class BasePreview {
	...
	protected constructor(context: ExtensionContext, uri: Uri, scheme: string) {
		...
		this._webMediaUri = Uri.file(path.join(context.extensionPath, 'media'));
		this._psUri = Uri.file(path.join(context.extensionPath, 'ps'));
